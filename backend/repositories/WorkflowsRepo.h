...
Json::Value getWorkflowWithSteps(const drogon::orm::DbClientPtr &db, long long id);

long long createWorkflow(const drogon::orm::DbClientPtr &db,
                          const std::string &name,
                          const std::string &description);

void updateWorkflow(const drogon::orm::DbClientPtr &db,
                     long long id,
                     const std::string &name,
                     const std::string &description);
...
// Each element must have "name", "type", and optionally "config" (object or string).
void replaceSteps(const drogon::orm::DbClientPtr &db,
