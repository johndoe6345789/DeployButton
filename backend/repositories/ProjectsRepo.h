...
// Returns an array of all projects, each with its last-run status/time joined
// in.
...
long long createProject(const drogon::orm::DbClientPtr& db,
                        ...
                        const std::string& repoUrl, long long workflowId);

void updateProject(const drogon::orm::DbClientPtr& db, long long id,
                   const std::string& name, const std::string& slug,
                   const std::string& repoUrl, long long workflowId);
